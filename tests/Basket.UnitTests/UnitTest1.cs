...
namespace Basket.UnitTests
{
    ...
    public class BasketControllerTests
    {
        ...
        public void AddToCart_ValidRequest_ReturnsOk()
        {
            ...
            var item = new CartItemDTO { ProductId = productId, Quantity = 2 };

            var result = _controller.Add(userId, item);

            Assert.IsInstanceOfType(result, typeof(OkResult));
            _mockBasketService.Verify(s => s.Add(It.Is<CartItemDTO>(i => i.ProductId == productId && i.Quantity == 2), userId), Times.Once);
        }
        ...
        public void GetCartItems_ValidRequest_ReturnsItems()
        {
            ...
            var items = new List<CartItemDTO>
            {
                ...
                new() { ProductId = Guid.NewGuid().ToString(), Quantity = 3 }
            };
        ...
        public void UpdateCartItem_ValidRequest_ReturnsOk()
        {
            var userId = "user123";
            var productId = Guid.NewGuid().ToString();
            var item = new CartItemDTO { ProductId = productId, Quantity = 5 };

            var result = _controller.Update(userId, item);

            Assert.IsInstanceOfType(result, typeof(OkResult));
            _mockBasketService.Verify(s => s.Update(It.Is<CartItemDTO>(i => i.ProductId == productId && i.Quantity == 5), userId), Times.Once);
        }
        ...
        public void RemoveFromCart_ExceptionThrown_ReturnsInternalServerError()
        {
            var userId = "user123";
            var productId = Guid.NewGuid().ToString();
            _mockBasketService.Setup(s => s.Remove(It.IsAny<string>(), It.IsAny<string>())).Throws(new Exception("Error removing from cart"));
