...
describe("AuthService", () => {
  ...
  it("should recover password by user ID", () => {
    ...
    authService.recoverPassword(userId).subscribe({
      next: () => {
        expect(true).toBe(true);
      },
      error: (error) => {
        fail("Expected no error, but got one: " + error);
      },
    });
  ...
  it("should handle error when recovering password fails", () => {
    const mockTokenResponse = {
      access_token: "mockAccessToken",
      refresh_token: "mockRefreshToken",
      expires_in: 3600,
    ...
    authService.recoverPassword("1234").subscribe({
      next: () => fail("Expected error, but password recovery succeeded"),
      error: (error) => {
    ...
    const req = httpMock.expectOne(
      "http://localhost:8070/admin/realms/eshop/users/1234/send-verify-email"
    );
    expect(req.request.method).toBe("PUT");
